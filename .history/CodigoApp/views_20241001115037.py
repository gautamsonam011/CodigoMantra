...
def user_signup_view(request):
    ...
    BlogUserForm = forms.BlogUserForm()
  
    mydict={'UserForm':UserForm,'BlogUserForm':BlogUserForm}
    if request.method=='POST':
        UserForm=forms.UserForm(request.POST)
        BlogUserForm=forms.BlogUserForm(request.POST,request.FILES)
        if UserForm.is_valid() and BlogUserForm.is_valid():
            user=UserForm.save()
            user.set_password(user.password)
            user.save()
            blogUser=BlogUserForm.save(commit=False)
            blogUser.user=user
...
def bloger_dashboard_view(request):
    ...
    return render(request,'Codigo/patient-dashboard.html',context=mydict)
...
def blog_dashboard_view(request):
    ...
    return render(request,'Codigo/dashboard.html',context=mydict)
